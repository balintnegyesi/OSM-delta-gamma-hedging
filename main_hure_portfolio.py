...
from lib.Hure.systemHure import systemHure as BSDESolver
...
def main(argv):
    ...
    if config.eqn_config.discretization_config.theta_y == 0.5:
        FLAGS.log_dir = './logs/systemHure_theta0p5/' + FLAGS.exp_name
    elif config.eqn_config.discretization_config.theta_y == 1:
        FLAGS.log_dir = './logs/systemHure_theta1/' + FLAGS.exp_name
    elif config.eqn_config.discretization_config.theta_y == 0:
        FLAGS.log_dir = './logs/systemHure_theta0/' + FLAGS.exp_name
    else:
    ...
    if config.net_config.is_report_t0_errors:
        ...
        if not bsde_solver.is_nn_at_t0:
            ...
            y_est = bsde_solver.Y[0](x0, False).numpy()
        try:
        ...
        if bsde.is_y_theoretical:
            ...
            y_error_abs = np.abs((y_anal - y_est))[0, 0]
            y_error_rel = np.abs((y_anal - y_est) / y_anal)[0, 0]
            try:
